      ...
      <div className="relative flex items-center gap-2" ref={ref}>
        ...
        {open && (
          <div className="absolute right-0 top-10 w-40 bg-white text-black rounded shadow-lg border z-50 text-sm">
            {isAdmin && (
              <>
                <div className="px-4 py-2 text-gray-600 font-semibold text-xs">User Management</div>
                <hr />
                <button
                  onClick={() => { setOpen(false); nav("/admin/role"); }}
                ...
                <hr />
                <button
                  onClick={() => { setOpen(false); nav("/admin/user"); }}
                  className="w-full text-left px-4 py-2 hover:bg-gray-100"
                >
                  User
                </button>
